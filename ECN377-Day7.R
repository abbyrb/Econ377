...
sdX <- sqrt(VarX)      #=0.5        # sqrt(EX - EX^2)
sdY <- sqrt(VarY)      #=0.5         # sqrt(EY - EY^2)
Cor <- Cov / (sdX*sdY) #=0.6              # Cor = Cov / (sdX * sdY)   -- should come out to 0.6
...
wage1$wage[is12] #returns wages of those who do have 12 years of education
w12  <- wage1$wage[is12]   # put the Boolean in [ ] to SELECT those people's wages     (wage1$wage[is12])
mean(w12)    #=5.371364      # E[wage | educ = 12] = the average of those wages          (mean(w12))
## Now educ == 16 (college), the same idea in one line:
mean(wage1$wage[wage1$educ ==16])     #=8.041618; wages for just those with a college education      # E[wage | educ = 16]
